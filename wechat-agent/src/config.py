...
class LoggerConfig(BaseModel):
    ...
    filename: str = Field(default="wechat-agent.log", description="日志文件名")
...
class ChatTransferConfig(BaseModel):
    ...
    chat: str = Field(description="微信会话名称（wxauto AddListenChat 的 nickname）")
...
def load_config_from_args() -> Config:
    ...
    parser = argparse.ArgumentParser(description="wechat-agent: wxauto → NATS publisher")
    parser.add_argument(
        "--config",
        type=str,
        default="config.yaml",
